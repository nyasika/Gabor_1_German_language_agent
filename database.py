...
def _client() -> Client:
    ...
    if not url or not key:
        raise EnvironmentError(
            f"Hiányzó env változó: {'SUPABASE_URL' if not url else 'SUPABASE_KEY'}"
        )
...
class Database:
    ...
    def add_tokens(self, tokens_in: int, tokens_out: int) -> None:
        try:
            ...
            pass  # column may not exist on older DB – silently ignore
...
-- Ha már létezik a tábla, futtasd ezt is:
ALTER TABLE user_state ADD COLUMN IF NOT EXISTS tokens_in INTEGER DEFAULT 0;
ALTER TABLE user_state ADD COLUMN IF NOT EXISTS tokens_out INTEGER DEFAULT 0;
