...
_tokens_in: int = 0
_tokens_out: int = 0
_tokens_cache_read: int = 0
_tokens_cache_write: int = 0


def _track_usage(response) -> None:
    global _tokens_in, _tokens_out, _tokens_cache_read, _tokens_cache_write
    u = response.usage
    _tokens_in += getattr(u, "input_tokens", 0)
    _tokens_out += getattr(u, "output_tokens", 0)
    _tokens_cache_read += getattr(u, "cache_read_input_tokens", 0)
    _tokens_cache_write += getattr(u, "cache_creation_input_tokens", 0)


def flush_tokens() -> tuple[int, int]:
    """Return accumulated (input_tokens, output_tokens) and reset counters."""
    global _tokens_in, _tokens_out, _tokens_cache_read, _tokens_cache_write
    t_in = _tokens_in + _tokens_cache_write
    t_out = _tokens_out
    _tokens_in = _tokens_out = _tokens_cache_read = _tokens_cache_write = 0
    return t_in, t_out
...
def _extract_json(text: str) -> str:
    """Strip markdown fences and return the outermost JSON structure.

    Picks whichever of { or [ appears first in the text, so an array
    response like [{...}, {...}] is not mistakenly parsed as the first
    element object.
    """
    text = text.strip()
    ...
    for start, start_char, end_char in sorted(candidates):
        depth = 0
...
def generate_session(
    ...
    type_prompts = {
        "lueckentext": f"""Generálj 6 Lückentext feladatot a Linie B1.2 {chapter}. fejezetének témájában.
A mondatok legyenek természetes, hétköznapi kontextusban (pl. párbeszéd, levél, újságcikk részlet).
Vegyítsd a szókincset és a grammatikát – ne tiszta grammatika-drill legyen!
A hiányzó rész lehet: szó, igealak, prepozíció, kötőszó vagy rövid szókapcsolat.
Jelöld a hiányzó részt ___ jellel a mondatban. A correct_answer az egyetlen helyes kitöltés legyen.""",
...
A szavak legyenek B1-B2 szintűek, hasznosak a mindennapokban.""",
        ...
        "satzstruktur": """Generálj 8 mondatszerkezet-gyakorlatot a következő témákban:
- V2-szabály (ige a 2. pozícióban főmondatban)
- Inverzió (ha nem az alany nyit, az ige és alany felcserélődik)
- Satzklammer (segédige 2. pozícióban, főnévi igenév/Partizip a végén)
- TeKaMoLo sorrend (temporal → kausal → modal → lokal)
- Nebensatz szórend (ragozott ige a végén)
- `nicht` helyes pozíciója

Típusok: 4 fill_in (a mondatba illeszd a hiányzó igét/mondatrészt a helyes pozícióba),
         4 multiple_choice (4 teljes mondat közül válaszd ki a helyes szórendűt).
A fill_in mondatokban ___ jelöli a hiányzó részt.
A multiple_choice opcióknál mind a 4 opció teljes mondat legyen – csak egy helyes szórendű.""",
    }
...
{{
  ...
  "exercises": [
    ...
    if isinstance(result, list):
        return {"session_type": session_type, "exercises": result}
