...
def clear_session():
    for key in ["session_data", "session_type", "session_answers",
                ...
                "session_due_vocab", "pending_session_type",
                "pending_due_vocab", "pending_chapter", "grammar_intro_topic"]:
...
def render_sidebar(state: dict):
    with st.sidebar:
        ...
        level = assessment_results.get("overall_level", "")
        if level:
...
Kb. **10–15 perc**, 20 kérdés.
    ...
    if "assessment_results" not in st.session_state:
        ...
        t_in, t_out = agent.flush_tokens()
        db.add_tokens(t_in, t_out)
...
def page_home(state: dict):
    ...
    session_items = list(SESSION_TYPES.items())
    for row in [session_items[:3], session_items[3:]]:
        cols = st.columns(3)
        for i, (stype, info) in enumerate(row):
            with cols[i]:
...
def _start_session(stype: str, state: dict, due_vocab: list):
    ...
    if stype == "satzstruktur":
        topic = "Satzstruktur"
    else:
        weakest = db.get_weakest_category()
        topic = weakest or CHAPTER_GRAMMAR_MAP.get(chapter, "Konjunktiv II")

    st.session_state.pending_session_type = stype
    st.session_state.pending_due_vocab = due_vocab
    st.session_state.pending_chapter = chapter
    st.session_state.grammar_intro_topic = topic
    go("grammar_intro")


def _generate_and_start(stype: str, chapter: int, due_vocab: list):
    ...
    st.session_state.session_due_vocab = due_vocab if stype == "vokabeln" else []
    go("session")
...
def page_grammar_intro():
    stype = st.session_state.get("pending_session_type", "grammatik")
    topic = st.session_state.get("grammar_intro_topic", "")
    chapter = st.session_state.get("pending_chapter", 1)
    due_vocab = st.session_state.get("pending_due_vocab", [])
    info = SESSION_TYPES.get(stype, {})

    st.title(f"{info.get('icon', '📚')} Nyelvtani összefoglaló")
    st.caption(f"Gyakorlat típusa: **{info.get('label', stype)}**")

    if stype == "vokabeln":
        ch_data = CHAPTERS.get(chapter, {})
        st.subheader(f"Fejezet {chapter}: {ch_data.get('name', '')}")
        st.markdown(f"**Grammatikai fókusz:** {ch_data.get('grammar_focus', '')}")
        key_vocab = ch_data.get("key_vocab", [])
        if key_vocab:
            st.markdown("**Fejezet kulcsszavai:**")
            cols = st.columns(2)
            for i, w in enumerate(key_vocab):
                cols[i % 2].markdown(f"- *{w}*")
        if due_vocab:
            st.markdown(f"**Ma esedékes ismétlés:** {len(due_vocab)} szó")

    elif stype == "schreiben":
        ch_data = CHAPTERS.get(chapter, {})
        st.subheader(f"Fejezet {chapter}: {ch_data.get('name', '')}")
        ...
        if topics:
            ...
            for t in topics:
                st.markdown(f"- {t}")
        st.markdown("**Hasznos kötőszavak fogalmazáshoz:**")
        st.markdown("""
| Kötőszó | Jelentés |
|---------|---------|
| außerdem | ráadásul, ezenfelül |
| deshalb / deswegen | ezért |
| trotzdem | ennek ellenére |
| allerdings | azonban, viszont |
...
""")
        ...
        expl = GRAMMAR_EXPLANATIONS.get(topic)
        if expl:
            st.subheader(expl["title"])
            st.markdown(expl["content"])
            if expl.get("tip"):
                st.info(expl["tip"])
        else:
            st.info(f"Fókusz téma: **{topic}**")

    st.markdown("---")
    if st.button("▶ Kezdjük a gyakorlást!", type="primary", use_container_width=True):
        _generate_and_start(stype, chapter, due_vocab)
...
def page_session():
    ...
    if q_idx >= len(exercises):
        ...
        t_in, t_out = agent.flush_tokens()
        db.add_tokens(t_in, t_out)
        duration = max(1, int((datetime.now() - st.session_state.session_start).total_seconds() / 60))
        db.save_session({
            "session_type": stype,
        ...
        # SM-2: update review intervals for vocabulary words
        if stype == "vokabeln":
            wrong_answers = {c.get("correct", "").lower() for c in evaluation.get("corrections", [])}
            for word in st.session_state.get("session_due_vocab", []):
                word_id = word.get("id")
                if word_id:
                    german = word.get("german", "").lower()
                    is_correct = not any(german in w for w in wrong_answers)
                    db.update_vocabulary_review(word_id, correct=is_correct)
...
def page_settings(state: dict):
    ...
    st.caption(
        "A GitHub Actions naponta 2× elküldi az emlékeztetőt erre a címre (07:00 és 19:00). "
...
def main():
    ...
    if page == "grammar_intro" and "pending_session_type" in st.session_state:
        page_grammar_intro()
    elif page == "session" and "session_data" in st.session_state:
        page_session()
    elif page == "stats":
        page_stats()
    elif page == "settings":
